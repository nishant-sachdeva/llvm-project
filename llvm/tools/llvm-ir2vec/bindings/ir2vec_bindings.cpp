...
public:
  ...
  TripletResult generateTriplets() { return Tool->getTriplets(); }
};
...
PYBIND11_MODULE(py_ir2vec, m) {
  ...
  py::class_<Triplet>(m, "Triplet")
		...
		.def(py::init<unsigned, unsigned, unsigned>(),
			py::arg("head"), py::arg("tail"), py::arg("relation"))
		.def_readonly("head", &Triplet::Head)
		.def_readonly("tail", &Triplet::Tail)
		.def_readonly("relation", &Triplet::Relation);

  // TripletResult struct
  py::class_<TripletResult>(m, "TripletResult")
		.def(py::init<>())
		.def_readonly("max_relation", &TripletResult::MaxRelation)
		.def_readonly("triplets", &TripletResult::Triplets);
  ...
  py::class_<PyIR2VecTool>(m, "IR2VecTool")
      ...
      "Returns: list[tuple[str, numpy.ndarray[float64]]]")
		.def("generateTriplets", &PyIR2VecTool::generateTriplets,
			...
			"Returns: TripletResult with max_relation and list of triplets");
