...
# Find .so file
SO_FILE=$(find "${BUILD_LLVM}/lib" -name "py_mir2vec*.so" | head -n1)
if [ -z "$SO_FILE" ]; then
    echo "ERROR: py_mir2vec*.so not found in ${BUILD_LLVM}/lib/"
...
mkdir -p "${TEMP_BUILD}/py_mir2vec"
cp "${SO_FILE}" "${TEMP_BUILD}/py_mir2vec/"
cp "${BINDINGS_SOURCE}/__init__.py" "${TEMP_BUILD}/py_mir2vec/"
